<x-app-layout>
    <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
        ...
        @endif
        <div class="relative">
            <h1 class="text-2xl md:text-2xl text-slate-800 dark:text-slate-100 font-bold mb-4">Rejected Loans
            </h1>
        </div>
                ...
                <table class="w-full border-collapse text-left text-sm rounded-md overflow-hidden border border-black">
                    <thead class="bg-bgbody-100 rounded-2xl">
                        <tr class="text-fonts-100 font-extrabold">
                            ...
                            <th class="p-4 text-fonts-100 font-normal">Rejection Date</th>
                            <th class="p-4 text-fonts-100 font-normal">Note</th>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200 dark:divide-gray-700">
                        @foreach ($lists as $list)
                            <tr class="text-fonts-200">
                                <td class="p-4 text-sm font-medium">{{ $list->id }}</td>
                                <td class="p-4 text-sm">{{ $list->customer->first_name }} {{ $list->customer->last_name }}</td>
                                <td class="p-4 text-sm">
                                    {{ $list->customer->house }} {{ $list->customer->street }}
                                    {{ $list->customer->bry->barangay_name }}
                                    {{ $list->customer->cty->city_town }}
                                </td>
                                <td class="p-4 text-sm">{{ number_format($list->principal_amount, 2) }}</td>
                                <td class="p-4 text-sm">{{ $list->updated_at->format('M d, Y') }}</td>
                                <td class="p-4 text-sm">{{ $list->note }}</td>
                                <td class="p-4 text-sm">
                                    <div class="flex items-center gap-x-4">
                                        <button class="text-primary transition duration-200 hover:text-indigo-500 focus:outline-none">
                                        ...
                                        <a href="{{ route('denomination.show', $list->id) }}"
                                            class="text-primary transition duration-200 hover:text-indigo-500 focus:outline-none">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
