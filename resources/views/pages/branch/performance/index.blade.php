<x-app-layout>
    <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
            ...
            @endif
            <div class="relative">
                <h1 class="text-2xl md:text-2xl text-fonts-200 dark:text-slate-100 font-bold mb-4">Performance Record
                </h1>
            </div>
                        ...
                        <table class="w-full border-collapse text-left text-sm rounded-md overflow-hidden border border-black">
                            <thead class="bg-bgbody-100 rounded-2xl">
                                <tr class="text-fonts-100 font-extrabold">
                                    <th class="p-4 text-fonts-100 font-normal">Branch Manager</th>
                                    <th class="p-4 text-fonts-100 font-normal">Branch Location</th>
                                    <th class="p-4 text-fonts-100 font-normal">Month</th>
                                    <th class="p-4 text-fonts-100 font-normal">Loans Disbursed</th>
                                    <th class="p-4 text-fonts-100 font-normal">Collection Efficiency</th>
                                    <th class="p-4 text-fonts-100 font-normal">New Clients Acquired</th>
                                    <th class="p-4 text-fonts-100 font-normal">Customer Retention</th>
                                    <th class="p-4 text-fonts-100 font-normal">Target Achievement</th>
                                </tr>
                            </thead>
                            ...
                            <tbody class="text-gray-600 font-semibold text-sm">
                                @foreach ($branch_managers as $list)
                                    <tr class="border-b hover:bg-gray-50">
                                        <td class="px-4 py-4 text-sm dark:text-gray-200 whitespace-nowrap">{{ $list->name }}</td>
                                        <td class="px-4 py-4 text-sm dark:text-gray-200 whitespace-nowrap">{{ $list->branch->location }}</td>
                                        <td class="px-4 py-4 text-sm text-gray-500 dark:text-gray-300 whitespace-nowrap">{{ now()->startOfMonth()->format('F') }}</td>
                                        <td class="px-4 py-4 text-sm text-gray-500 dark:text-gray-300 whitespace-nowrap">{{ $list->loans->count() }}</td>
                                        <td class="px-4 py-4 text-sm text-gray-500 dark:text-gray-300 whitespace-nowrap">0</td>
                                        <td class="px-4 py-4 text-sm text-gray-500 dark:text-gray-300 whitespace-nowrap">{{ $list->customers->count() }}</td>
                                        <td class="px-4 py-4 text-sm text-gray-500 dark:text-gray-300 whitespace-nowrap">{{ $list->customers->count() }}</td>
                                        <td class="px-4 py-4 text-sm text-gray-500 dark:text-gray-300 whitespace-nowrap">0</td>
                                    </tr>
                                @endforeach
