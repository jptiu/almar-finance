<x-app-layout>
    <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
        ...
        @endif
        <div class="relative">
            <h1 class="text-2xl md:text-2xl text-slate-800 dark:text-slate-100 font-bold mb-4">Breakdown of Cash Bills
            </h1>
        </div>
        ...
        <section class="container">
            <div class="p-6 w-full max-w-[1500px] mx-auto bg-white rounded-lg border border-bgbody-200">
                <div class="sm:flex sm:justify-between sm:items-center mb-4">
                    ...
                    <a href="{{ route('breakdown.create') }}" class="btn bg-indigo-500 hover:bg-indigo-600 text-white rounded-full px-4 py-3 flex items-center">
                ...
                <table class="w-full border-collapse text-left text-sm rounded-md overflow-hidden border border-black">
                    <thead class="bg-bgbody-100 rounded-2xl">
                        <tr class=" text-fonts-100 font-extrabold">
                            ...
                            <th class="p-4 text-fonts-100 font-normal">Cashier</th>
                        </tr>
                    </thead>
                    <tbody class="bg-white divide-y font-semibold text-sm divide-gray-200 dark:divide-gray-500 dark:bg-gray-900">
                        @foreach ($lists as $list)
                            <tr class="border-b hover:bg-gray-50">
                                <td class="p-4">{{ $list->ref_no }}</td>
                                <td class="p-4">{{ $list->date }}</td>
                                <td class="p-4">{{ $list->total_amount }}</td>
                                <td class="p-4">{{ $list->user_id }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
